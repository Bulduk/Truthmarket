...
import { Music, Volume2, TrendingDown, Flame, Target, Play } from 'lucide-react';
import Image from 'next/image';

const POSITIONS: { id: Position; label: string; icon: React.ElementType; desc: string; multiplier: string; color: string }[] = [
  { id: 'resonate', label: 'RESONATE', icon: Volume2, desc: 'Rezonans', multiplier: '2.3x', color: '#ff007f' },
  { id: 'fade', label: 'FADE', icon: TrendingDown, desc: 'Söner', multiplier: '3.8x', color: '#ff4757' },
  { id: 'trend', label: 'TREND', icon: Flame, desc: 'Trende', multiplier: '9.2x', color: '#F0B90B' },
  { id: 'niche', label: 'NICHE', icon: Target, desc: 'Niş', multiplier: '4.5x', color: '#00e5ff' },
];

export function VibeTradeModal() {
  ...
  const getActiveColor = () => {
    if (!selectedPosition) return '#ff007f'; // Default VIBE active color
    return POSITIONS.find(p => p.id === selectedPosition)?.color || '#ff007f';
  };
    ...
    <div className="flex flex-col h-full text-[#E8ECF4]">
      ...
      <div className="flex items-start gap-4 mb-6 pb-6 border-b border-white/5">
        <div className="w-16 h-16 rounded-xl overflow-hidden relative shrink-0 bg-[#1e2329] border border-white/10 flex items-center justify-center">
          <Music className="w-8 h-8 text-[#ff007f]" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold text-[#ff007f] bg-[#ff007f]/10 px-2 py-0.5 rounded-full border border-[#ff007f]/20">
              🎵 VIBE MARKET
            </span>
          </div>
          <h3 className="text-base font-bold text-white truncate">Weekend vibes playlist</h3>
          <p className="text-sm text-[#9BA3B8] truncate mt-0.5">👤 CreatorName • ⏱ 7 gün kaldı</p>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="bg-[#181A20] rounded-xl p-4 border border-white/5">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-xs text-[#9BA3B8]">Mevcut Dinlenme</span>
                <div className="text-sm font-bold text-white mt-1">45,102</div>
              </div>
              <div>
                <span className="text-xs text-[#9BA3B8]">Playlist Eklenmesi</span>
                <div className="text-sm font-bold text-[#ff007f] mt-1">3,421</div>
              </div>
            </div>
          </div>
      ...
      {step === 2 && (
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="bg-[#131720] p-4 rounded-xl border border-[#ff007f]/20 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#ff007f]/5 rounded-full blur-2xl -mt-10 -mr-10"></div>
          ...
          <div className="flex gap-3">
            ...
            <Button 
              className="flex-[2] text-black font-bold h-12 disabled:opacity-50"
              style={{ 
      ...
      {step >= 3 && (
        <div className="flex-1 flex flex-col min-h-0 space-y-6 items-center justify-center text-center py-6">
          <Play className="w-16 h-16 text-[#ff007f] animate-pulse mb-4" />
