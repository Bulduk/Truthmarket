...
import { Target, XCircle, Clock, Snail, LineChart, Play } from 'lucide-react';

const POSITIONS: { id: Position; label: string; icon: React.ElementType; desc: string; multiplier: string; color: string }[] = [
  { id: 'sharp', label: 'SHARP', icon: Target, desc: 'Keskin', multiplier: '1.9x', color: '#00D68F' },
  { id: 'dull', label: 'DULL', icon: XCircle, desc: 'Yanılır', multiplier: '4.8x', color: '#f6465d' },
  { id: 'early', label: 'EARLY', icon: Clock, desc: 'Erken', multiplier: '8.3x', color: '#F0B90B' },
  { id: 'late', label: 'LATE', icon: Snail, desc: 'Geç', multiplier: '6.1x', color: '#9BA3B8' },
];

export function EdgeTradeModal() {
  ...
  const getActiveColor = () => {
    if (!selectedPosition) return '#3b82f6';
    return POSITIONS.find(p => p.id === selectedPosition)?.color || '#3b82f6';
  };
    ...
    <div className="flex flex-col h-full text-[#E8ECF4]">
      ...
      <div className="flex items-start gap-4 mb-6 pb-6 border-b border-white/5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold text-[#3b82f6] bg-[#3b82f6]/10 px-2 py-0.5 rounded-full border border-[#3b82f6]/20">
              📊 EDGE MARKET
            </span>
          </div>
          <h3 className="text-base font-bold text-white truncate">Macro Q3 Outlook</h3>
          <p className="text-sm text-[#9BA3B8] truncate mt-0.5">👤 AnalystName • ⏱ 30 gün kaldı</p>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="bg-[#181A20] rounded-xl p-4 border border-white/5">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-xs text-[#9BA3B8]">Analist Doğruluğu</span>
                <div className="text-sm font-bold text-white mt-1">%71</div>
              </div>
              <div>
                <span className="text-xs text-[#9BA3B8]">Alpha Skoru</span>
                <div className="text-sm font-bold text-[#3b82f6] mt-1">8.3/10</div>
              </div>
            </div>
          </div>
          ...
          <Button 
            ...
            style={{ 
              ...
              color: selectedPosition === 'early' || selectedPosition === 'sharp' ? '#000' : '#fff'
            }}
      ...
      {step === 2 && (
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="bg-[#131720] p-4 rounded-xl border border-[#3b82f6]/20 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#3b82f6]/5 rounded-full blur-2xl -mt-10 -mr-10"></div>
      ...
      {step >= 3 && (
        <div className="flex-1 flex flex-col min-h-0 space-y-6 items-center justify-center text-center py-6">
          <Play className="w-16 h-16 text-[#3b82f6] animate-pulse mb-4" />
