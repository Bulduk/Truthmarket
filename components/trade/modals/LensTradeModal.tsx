...
import { Camera, Trophy, Hourglass, Rocket, Archive, Play } from 'lucide-react';

const POSITIONS: { id: Position; label: string; icon: React.ElementType; desc: string; multiplier: string; color: string }[] = [
  { id: 'iconic', label: 'ICONIC', icon: Trophy, desc: 'İkonik', multiplier: '2.5x', color: '#FF8C00' },
  { id: 'fleeting', label: 'FLEETING', icon: Hourglass, desc: 'Geçici', multiplier: '4.1x', color: '#ff4757' },
  { id: 'viral', label: 'VIRAL', icon: Rocket, desc: 'Viral', multiplier: '11.3x', color: '#00e5ff' },
  { id: 'archive', label: 'ARCHIVE', icon: Archive, desc: 'Arşiv', multiplier: '5.8x', color: '#9BA3B8' },
];

export function LensTradeModal() {
  ...
  const getActiveColor = () => {
    if (!selectedPosition) return '#FF8C00'; 
    return POSITIONS.find(p => p.id === selectedPosition)?.color || '#FF8C00';
  };
    ...
    <div className="flex flex-col h-full text-[#E8ECF4]">
      ...
      <div className="flex items-start gap-4 mb-6 pb-6 border-b border-white/5">
        <div className="w-16 h-16 rounded-xl overflow-hidden relative shrink-0 bg-[#1e2329] border border-white/10 flex items-center justify-center">
          <Camera className="w-8 h-8 text-[#FF8C00]" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold text-[#FF8C00] bg-[#FF8C00]/10 px-2 py-0.5 rounded-full border border-[#FF8C00]/20">
              📸 LENS MARKET
            </span>
          </div>
          <h3 className="text-base font-bold text-white truncate">Sunset over Miami</h3>
          <p className="text-sm text-[#9BA3B8] truncate mt-0.5">👤 Photographer • ⏱ 30 gün kaldı</p>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="bg-[#181A20] rounded-xl p-4 border border-white/5">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-xs text-[#9BA3B8]">Kaydetme Sayısı</span>
                <div className="text-sm font-bold text-white mt-1">1,240</div>
              </div>
              <div>
                <span className="text-xs text-[#9BA3B8]">Görsel Etki Skoru</span>
                <div className="text-sm font-bold text-[#FF8C00] mt-1">84/100</div>
              </div>
            </div>
          </div>
          ...
          <Button 
            ...
            style={{ 
              ...
              color: selectedPosition === 'viral' || selectedPosition === 'iconic' ? '#000' : '#fff'
            }}
      ...
      {step === 2 && (
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="bg-[#131720] p-4 rounded-xl border border-[#FF8C00]/20 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#FF8C00]/5 rounded-full blur-2xl -mt-10 -mr-10"></div>
      ...
      {step >= 3 && (
        <div className="flex-1 flex flex-col min-h-0 space-y-6 items-center justify-center text-center py-6">
          <Play className="w-16 h-16 text-[#FF8C00] animate-pulse mb-4" />
