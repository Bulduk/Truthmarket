...
import { Zap, Activity, TrendingUp, TrendingDown, Snowflake, Flame, Play } from 'lucide-react';

const POSITIONS: { id: Position; label: string; icon: React.ElementType; desc: string; multiplier: string; color: string }[] = [
  { id: 'amplify', label: 'AMPLIFY', icon: Flame, desc: 'Yayılır', multiplier: '2.1x', color: '#c084fc' }, // Neon purple
  { id: 'dampen', label: 'DAMPEN', icon: Snowflake, desc: 'Söner', multiplier: '3.4x', color: '#848E9C' }, // Gray
  { id: 'spike', label: 'SPIKE', icon: Zap, desc: 'Patlar', multiplier: '12.5x', color: '#F0B90B' }, // Gold
  { id: 'fade', label: 'FADE', icon: TrendingDown, desc: 'Yavaş', multiplier: '6.2x', color: '#FF4757' }, // Red
];

export function ReachTradeModal() {
    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="flex items-start gap-4 pb-4 border-b border-[#252B3B]">
        <div className="relative w-16 h-20 rounded-lg overflow-hidden bg-[#1E2433] flex-shrink-0 flex items-center justify-center">
          <Play className="w-6 h-6 text-[#9BA3B8]"/>
          {/* Mock thumbnail can go here */}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] font-bold tracking-widest text-[#c084fc] bg-[rgba(138,43,226,0.1)] px-2 py-0.5 rounded-sm">REACH MARKET</span>
          </div>
          <h3 className="text-sm font-bold text-white leading-tight mb-2 truncate">This AI agent is building its own apps...</h3>
          <p className="text-xs text-[#9BA3B8] flex items-center gap-2">
            <span>@techguru</span>
            <span>•</span>
            <span className="text-[#F0B90B]">⏱ 2 gün kaldı</span>
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#131720] border border-[#252B3B] p-4 rounded-xl space-y-3">
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#9BA3B8]">Mevcut görüntülenme</span>
              <span className="text-white font-mono font-bold">124K</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#9BA3B8]">Viral coefficient</span>
              <span className="text-white font-mono font-bold">2.3</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#9BA3B8]">24s değişim</span>
              <span className="text-[#00D68F] font-mono font-bold">+18%</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#9BA3B8]">Signal strength</span>
              <div className="flex items-center gap-2">
                <div className="w-24 h-1.5 bg-[#1E2433] rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-[#c084fc] to-[#00D68F]" style={{ width: '73%' }}></div>
                </div>
                <span className="text-white font-mono font-bold text-xs">73/100</span>
              </div>
            </div>
          </div>

          <Button 
            className="w-full bg-[#c084fc] hover:bg-[#a855f7] text-white font-bold h-12 shadow-[0_0_15px_rgba(138,43,226,0.3)] disabled:opacity-50 disabled:shadow-none"
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <div className="p-4 bg-[rgba(138,43,226,0.05)] border border-[rgba(138,43,226,0.2)] rounded-xl space-y-2">
      ...
      {/* Confirmation and Success states placeholder for now */}
      {step === 3 && (
        <div className="text-center py-8 space-y-4">
          <div className="w-16 h-16 rounded-full bg-[#00D68F]/20 flex items-center justify-center mx-auto mb-4">
            <Activity className="w-8 h-8 text-[#00D68F] animate-pulse" />
          </div>
